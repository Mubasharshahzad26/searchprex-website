    ...
    <main className="bg-transparent min-h-screen">
      ...
      <section className="bg-[#eeeef5] relative overflow-hidden pt-28 pb-0">
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-0">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            {/* ✅ FIXED: text-white → text-[#0a0f2e] */}
            <h1 className="text-5xl sm:text-6xl font-black text-[#0a0f2e] mb-5 leading-tight">SearchPrex Blog</h1>
            {/* ✅ FIXED: text-blue-200 → text-[#64748b] */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0 border border-[#e5e7eb] rounded-xl overflow-hidden">
            {categories.map((cat, i) => (
              <button key={cat.slug}
                ...
                className={`text-left p-8 border-[#e5e7eb] transition-all hover:bg-white group ${i % 3 !== 2 ? "border-r" : ""} ${i < 3 ? "border-b" : ""} ${activeCategory === cat.label ? "bg-white shadow-inner" : "bg-[#eeeef5]"}`}
              >
      ...
      <section className="bg-[#eeeef5] py-14 border-b border-[#e5e7eb]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {/* ✅ FIXED: text-blue-300/60 → text-[#94a3b8] */}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {filtered.map((post, i) => (
                <motion.article key={post.slug} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="flex flex-col bg-white rounded-2xl border border-[#e5e7eb] overflow-hidden hover:shadow-lg transition-all p-6">
          ...
          <div className="flex items-center justify-center gap-2 mt-16 flex-wrap">
            {["First", "Previous", "1", "2", "3", "4", "...", "Last"].map((p) => (
              <button key={p} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${p === "1" ? "bg-[#0a0f2e] text-white" : "text-[#64748b] hover:text-[#0a0f2e] hover:bg-white border border-transparent hover:border-[#e5e7eb]"}`}>{p}</button>
            ))}
          </div>
        </div>
      </section>
