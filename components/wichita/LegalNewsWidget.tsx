...
const NEWS = [
  { tag: "Kansas Courts",  title: "Sedgwick County expands online filing for family law cases", date: "Jun 2026", color: PURPLE },
  { tag: "Local SEO",      title: "Google AI Overviews now citing local attorneys in legal queries — new study", date: "May 2026", color: GREEN },
  { tag: "Family Law",     title: "Kansas Supreme Court updates child support calculation guidelines", date: "May 2026", color: "#BA7517" },
  { tag: "Legal Tech",     title: "Law firms using structured schema see 2.3x more AI Overview appearances", date: "Apr 2026", color: PURPLE },
  { tag: "Wichita",        title: "Wichita Bar Association launches updated attorney directory with GBP links", date: "Mar 2026", color: GREEN },
  { tag: "Kansas Law",     title: "New Kansas statute affects spousal maintenance calculations in divorce cases", date: "Feb 2026", color: "#185FA5" },
];
    ...
    <aside style={{ fontFamily: FONT, position: "sticky", top: 24 }}>
      <div style={{ background: "#fff", border: `1px solid ${LINE}`, borderRadius: 14, overflow: "hidden" }}>
        ...
        <div style={{ padding: "6px 0" }}>
          {NEWS.map((item, i) => (
            <div key={i} style={{ padding: "12px 18px", borderBottom: i < NEWS.length - 1 ? `1px solid ${LINE}` : "none" }}>
              ...
              <p style={{ fontSize: 13, color: NAVY, lineHeight: 1.5, margin: 0, fontWeight: 600 }}>{item.title}</p>
            </div>
          ))}
        </div>
      ...
      <div style={{ marginTop: 12, background: `${PURPLE}10`, border: `1px solid ${PURPLE}30`, borderRadius: 12, padding: "14px 18px" }}>
        ...
        <div style={{ fontSize: 13.5, fontWeight: 700, color: NAVY, marginBottom: 6 }}>See where your firm ranks right now</div>
        <a href="https://calendly.com/contact-searchprex/30min" target="_blank" rel="noopener noreferrer"
          style={{ display: "block", textAlign: "center", background: PURPLE, color: "#fff", fontSize: 13, fontWeight: 700, padding: "9px 14px", borderRadius: 8, textDecoration: "none" }}>
          Book free call →
        </a>
      </div>
    </aside>
