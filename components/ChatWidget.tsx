...
export default function ChatWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      text: "Hi, I'm Mubashar Shahzad, founder of Searchprex. Looking for an SEO growth plan? Tell me a bit about your website or goals and I'll point you in the right direction.",
      isBot: true,
      timestamp: new Date(),
    },
  ]);
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div className="bg-[#0a0f2e] px-4 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/70">Verified SEO Expert &amp; Founder, Searchprex</p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                {messages.map((message) => (
                  <div key={message.id} className={`flex ${message.isBot ? "justify-start" : "justify-end"}`}>
                    <div className={`max-w-[80%] rounded-2xl px-4 py-2 ${message.isBot ? "bg-white text-[#374151] shadow-sm" : "bg-[#534AB7] text-white"}`}>
            ...
            {/* Privacy Note */}
            <div className="border-t border-[#e5e7eb] bg-white px-4 pt-3">
            ...
            <div className="bg-white p-4 pt-2">
              <form onSubmit={(e) => { e.preventDefault(); handleSend(inputValue); }} className="flex items-center gap-2">
                ...
                <button type="submit" disabled={!inputValue.trim()}
                  className="flex h-10 w-10 items-center justify-center rounded-full bg-[#534AB7] text-white transition-colors hover:bg-[#3C3489] disabled:opacity-50">
